	...
	switch (messageType)
	{
		...
		case 'calcPotential': 
			...
			let output = new Uint8ClampedArray(
				potKernel(
					message.data.potPosses, 
					message.data.sigmas, 
					message.data.periods, 
					message.data.potPosses.length, 
					config.pxOutputSize,
					config.viewSize,
				)
			);
...
function setup(_config) {
	...
	potKernel = gpu.createKernel(function(_positions, _sigmas, _periods, _posLength, _arrSize, _viewSize) {
		const epsilon = 10;
